...
## What are the CpGs with weird loglik?

```{r weirdCpG}
# res <- sapply(common_cpgs[1:1000], function(X) {
#   getLogLik_oneCpG_optimized(scaled_list_mat, 
#                            mu_jk_list, 
#                            sigma_k_list,
#                            j = X, 
...
# Warning: Non-finite loglik for cg00039385 in dataset PBMC_AfricanAmericanWarning: Non-finite loglik for cg00040013 in dataset PBMC_AfricanAmerican
...
datasetName = "PBMC_AfricanAmerican"
CpG = "cg00039385"

raw = readRDS(paste0("/mnt/old_user_accounts/p3/maria/PhD/Data/datasets/GEO/Raw_cleaned_beta_matrices_GEO/", datasetName))
transfo = readRDS(paste0("/mnt/old_user_accounts/p3/maria/PhD/Data/datasets/GEO/BMIQ + 10 PCs + age + sex OUTLIERS REMOVED/",datasetName, ".RDS"))

list(raw = raw[rownames(raw) %in% CpG,],
     transfoMaria = transfo[rownames(transfo) %in% CpG,])
```
...
```{r loglik_all}
set.seed(1234)
testCpGs20 <- c(sample(common_cpgs[common_cpgs %in% MariasCpGs$CpG], 10),
                sample(common_cpgs[!common_cpgs %in% MariasCpGs$CpG], 10))
testCpGs200 <- c(sample(common_cpgs[common_cpgs %in% MariasCpGs$CpG], 100),
                 sample(common_cpgs[!common_cpgs %in% MariasCpGs$CpG], 100))
testCpGs2000 <- c(sample(common_cpgs[common_cpgs %in% MariasCpGs$CpG], 1000),
                  sample(common_cpgs[!common_cpgs %in% MariasCpGs$CpG], 1000))

logLikData <- sapply(testCpGs200,
                     function(x){
                       getLogLik_oneCpG_optimized(scaled_list_mat, 
                                                  mu_jk_list, 
                                                  sigma_k_list,
...
## Check:
getLogLik_oneCpG_optimized(scaled_list_mat = scaled_list_mat, 
                           mu_jk_list = mu_jk_list, sigma_k_list = sigma_k_list,
                           j = common_cpgs[[1]], p0 = 0.95, p1 = 0.65, alpha = 0.01,
                           lambdas = lambdas) ==
  fun2optim(par = c(alpha = 0.01), scaled_list_mat = scaled_list_mat, 
            mu_jk_list = mu_jk_list, sigma_k_list = sigma_k_list,
            j = common_cpgs[[1]], p0 = 0.95, p1 = 0.65, lambdas = lambdas) 
```
...
system.time(res <- runOptim1CpG(CpG = testCpGs200[1]))
# user  system elapsed 
#  5.373   0.353   5.776 
res
...
system.time(res_allOptimAlpha <- getAllOptimAlpha(cpgvec = testCpGs200))
## 1795.180 sec for 200 CpGs
## 400000 *1795.180/200/60/60/24 ## 40 days to run all at this rate

ggplot(data.frame(alpha=res_allOptimAlpha, 
                  CpG=rownames(res_allOptimAlpha), 
                  Previous.results = ifelse(rownames(res_allOptimAlpha) %in% MariasCpGs$CpG, "hvCpG in Maria's study", "not picked up by Maria")),
       aes(x=CpG, y=alpha, fill = Previous.results)) +
  geom_point(pch=21, size = 3, alpha = .8) + scale_fill_manual(values = c("red", "grey"))+
  theme_bw() + theme(axis.text.x = element_blank()) +
  ylab("Probability alpha of being a hvCpG")
```
