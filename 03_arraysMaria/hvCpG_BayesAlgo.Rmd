...
getLogLik_oneCpG_optimized <- function(scaled_list_mat, mu_jk_list, sigma_k_list,
                                       j, lambda_start, p0, p1, alpha) {
  # Precompute all necessary values upfront
  lambda_vector <- setNames(rep(1.9, length(scaled_list_mat)), names(scaled_list_mat))
  datasets <- names(scaled_list_mat)
  log_P_Mj <- 0 ## initialisation
...
## Optimise over multiple parameters:

Let the parameter vector be:
* par[1] = alpha
* par[2:(K+1)] = lambda_vector (one lambda per dataset)

```{r, eval=FALSE}
# K is the number of datasets
K <- length(filtered_list_mat)

# Wrapper function for optim
loglik_wrapper <- function(par, scaled_list_mat, mu_jk_list, sigma_k_list, j, p0, p1) {
  alpha <- par[1]
  lambda_vector <- par[2:(K+1)]
...
# Initial values
init_alpha <- 0.01
init_lambda <- rep(1.9, K)
init_par <- c(init_alpha, init_lambda)

# Optionally, set bounds (e.g., alpha > 0, lambda > 0)
lower_bounds <- c(1e-5, rep(0.1, K))
upper_bounds <- c(1, rep(10, K))
...
optimal_alpha <- result$par[1]
optimal_lambda_vector <- result$par[2:(K+1)]
cat("Optimal alpha:", optimal_alpha, "\n")
cat("Optimal lambda_vector:", optimal_lambda_vector, "\n")
```
