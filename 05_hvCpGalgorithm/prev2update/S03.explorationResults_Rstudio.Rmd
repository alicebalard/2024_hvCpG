...
## Save it
write.table(cpg_names_all, "resultsDir/Mariads/cpg_names_all.tsv", row.names = F, quote = F, col.names = F)
...
sdselected <- function(cpglist, B = 1000) {
...
## Same, but scaling first

```{r}

rds_list_mat_scaled <- lapply(rds_list_mat, function(x){
  # 1. Clip beta values
  p <- pmin(pmax(x, 1e-6), 1 - 1e-6)
  # 2. Logit transformation in log2 scale
  log2(p / (1 - p))
})

sdselected <- function(cpglist, B = 1000) {
  results <- lapply(names(rds_list_mat_scaled), function(dataset_name) {
    k <- rds_list_mat_scaled[[dataset_name]]
    mat <- as.matrix(k[rownames(k) %in% cpglist, ])
...
## Distribution of DNAm values in array data

```{r}
hist(rds_list_mat$lung[,2], breaks = 100)
```
...
load("/home/alice/2024_hvCpG/05_hvCpGalgorithm/resultsDir/Mariads/results_MariasarraysREDUCED_3samples_15datasets_6906CpGs_0_8p0_0_65p1.RData")
