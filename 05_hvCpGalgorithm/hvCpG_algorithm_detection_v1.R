...
runAndSave <- function(my_list_mat, cpgvec, resultDir, optimMeth, NCORES, p0, p1) {
  ...
  # Clean variable names (remove possible invalid characters)
  var_mylist <- deparse(substitute(my_list_mat))
  var_cpgvec <- deparse(substitute(cpgvec))
  
  obj_name <- paste0("results_", optimMeth, "_",
                     var_mylist, "_",
                     var_cpgvec, "_",
                     as.character(p0), "p0_", as.character(p1), "p1")

  # Check if object name is valid (no dashes, spaces, etc.)
  obj_name <- gsub("[^[:alnum:]_]", "_", obj_name)

  assign(obj_name, result, envir = .GlobalEnv)

  # Ensure resultDir ends with a /
