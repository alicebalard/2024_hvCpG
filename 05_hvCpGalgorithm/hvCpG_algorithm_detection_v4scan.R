...
prepData <- function(analysis) {
  base_dirs <- list(
    testLocalPC = "~/Documents/Project_hvCpG/10X/",
  ...
  if (analysis %in% names(base_dirs)) {
    ...
    h5file <- file.path(mydir, "all_scaled_matrix.h5")
...
getAllOptimAlpha_parallel_batch <- function(cpgPos_vec, NCORES, p0, p1, prep, batch_size = 1000) {
  ...
  for (b in seq_along(batches)) {
    message(sprintf("📦 Loading batch %d / %d (%d CpGs)", b, length(batches), length(batches[[b]])))
    ## Read multiple CpGs at once from HDF5
    M_batch <- h5read(h5file, "scaled_matrix", index = list(NULL, batches[[b]]))
...
runAndSave <- function(analysis, cpgPos_vec, resultDir, NCORES, p0, p1, overwrite = FALSE, batch_size = 1000) {
