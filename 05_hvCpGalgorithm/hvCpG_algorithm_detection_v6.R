...
getLogPhv_oneCpG_byTissue <- function(Mdf, metadata, dataset_groups, ds_params, p0, p1) {
  ...
  for (k in datasets) { 
    ...
    dataset_logphv <- sum(log(norm_probs[,1]*p0)/(norm_probs[,1]*p0 + (norm_probs[,2]*(1-p0))))
...
getALL_LogPhv_oneCpG_byTissue_batch <- function(cpg_names_vec, NCORES, p0, prep, batch_size = 1000, Nds) {
  metadata       <- prep$metadata
  cpg_names_all  <- prep$cpg_names_all
  h5file         <- prep$h5file
  medsd_lambdas  <- prep$medsd_lambdas
...
runAndSave_tissueAnalysis <- function(analysis, cpg_names_vec, resultDir, NCORES, p0, overwrite = FALSE,
                            batch_size = 1000, dataDir, skipsave=FALSE, Nds=3, subsetMetadata = FALSE) {
  t <- Sys.time()
  prep <- prepData(analysis, dataDir, subsetMetadata)
  message("Preparing the data took ", round(Sys.time() - t), " seconds")
  
  obj_name <- paste0("results_bytissue_", analysis, "_", length(cpg_names_vec), "CpGs_", p0, "p0_", )
  obj_name <- gsub("[^[:alnum:]_]", "_", obj_name)
  ...
  result <- getALL_LogPhv_oneCpG_byTissue_batch(
    cpg_names_vec = cpg_names_vec, NCORES = NCORES,
    p0 = p0, prep = prep, batch_size = batch_size, Nds = Nds
  )
