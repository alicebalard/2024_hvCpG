...
## ******************************************* ##
## NB: select only 10k first CpGs for testing
cpg_46 <- head(cpg_46, 10000)
## ******************************************* ##
...
# Output directory
result_dir <- sprintf("/SAN/ghlab/epigen/Alice/hvCpG_project/code/2024_hvCpG/05_hvCpGalgorithm/resultsDir/Atlas/Atlas10X_TESTS/Atlas_batch%03d", task_id)
...
#for (i in seq(1000, 10000, 2000)){
#  print(paste0("test with batch size of ", i, " CpGs"))
#  system.time(runAndSave_fast(
#    analysis = "Atlas10X_test",
...
message("Now, run tissue algo:")

system.time(runAndSave_tissueAnalysis(
    analysis = "Atlas10X_tissue_test",
    cpg_names_vec = subset_cpgs,
    dataDir = data_dir,
    resultDir = result_dir,
    NCORES = myNthreads,
