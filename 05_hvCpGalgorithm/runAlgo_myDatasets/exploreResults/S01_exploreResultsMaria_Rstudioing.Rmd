...
sdselected <- function(cpglist, B = 1000, select3 = FALSE) {
  if (select3){
      # randomly sample 3 individuals (columns)
      selected_cols <- sample(seq_len(ncol(mat)), 3)
      mat <- mat[, selected_cols, drop = FALSE]
    }    
  
  results <- lapply(names(rds_list_mat), function(dataset_name) {
    k <- rds_list_mat[[dataset_name]]
    mat <- as.matrix(k[rownames(k) %in% cpglist, ])
...
df <- rbind(data.frame(medianSD = sdselected(cpglist = sub_cistrans_GoDMC_hvCpG_matched_control$hvCpG_name), type="hvCpG"),
            data.frame(medianSD = sdselected(cpglist = sub_cistrans_GoDMC_hvCpG_matched_control$controlCpG_name), type="mQTLcontrol"))
...
ggplot(df, aes(x = medianSD.dataset, y = medianSD.median_sd, fill = type)) +
  ...
  ggtitle("Array. All individuals")
```

## Same but we select 3 individuals randomly per dataset

```{r}
set.seed(1234)
df2 <- rbind(data.frame(medianSD = sdselected(cpglist = sub_cistrans_GoDMC_hvCpG_matched_control$hvCpG_name,
                                             select3 = TRUE), type="hvCpG"),
            data.frame(medianSD = sdselected(cpglist = sub_cistrans_GoDMC_hvCpG_matched_control$controlCpG_name, 
                                             select3 = TRUE), type="mQTLcontrol"))

ggplot(df2, aes(x = medianSD.dataset, y = medianSD.median_sd, fill = type)) +
  geom_pointrange(
    aes(ymin = medianSD.lower, ymax = medianSD.upper),
    shape = 21, size = 0.4, stroke = 0.5
  ) +
  ...
  xlab("") +
  ylab("Median SD ± 95% CI")+
  ggtitle("Array. 3 individuals randomly selected per dataset")
```
...
load("/home/alice/2024_hvCpG/05_hvCpGalgorithm/runAlgo_myDatasets/resultsDir/Mariads/results_Maria_6906CpGs_0_8p0_0_65p1.RData")
...
Brent = data.frame(CpG = cpg_names_all[as.numeric(row.names(results_Maria_pos2check_0_8p0_0_65p1))],
  ...
  mutate(ishvCpG = ifelse(CpG %in% Marias_hvCpGs$CpG, "hvCpGs", "controls"))
...
Brent %>%
  ...
  ggtitle("Considering all samples per datasets")+
  scale_fill_manual(values = c("black", "turquoise"))+
  scale_colour_manual(values = c("black", "turquoise"))

# "CpGs covered in >=15 datasets"


## Save for comparison with Atlas
write.table(Brent, "results_Maria_6906CpGs_0_8p0_0_65p1.tsv", quote = F, row.names = F)

table(is.na(Brent$alpha))

## Reduced version (only 3 samples)
load("/home/alice/2024_hvCpG/05_hvCpGalgorithm/runAlgo_myDatasets/resultsDir/Mariads/results_MariasarraysREDUCED_3samples_15datasets_6906CpGs_0_8p0_0_65p1.RData")

Brent2 = data.frame(CpG = rownames(results_MariasarraysREDUCED_3samples_15datasets_6906CpGs_0_8p0_0_65p1),
                    alpha = as.vector(results_MariasarraysREDUCED_3samples_15datasets_6906CpGs_0_8p0_0_65p1)) %>%  
  mutate(ishvCpG = ifelse(CpG %in% Marias_hvCpGs$CpG, "hvCpGs", "controls"))
...
Brent2 %>%
  ...
  theme(legend.position = "none", axis.title.x = element_blank()) +
  ylab("Probability of being a hvCpG") +
  ggtitle("Considering 3 samples per datasets")+
  scale_fill_manual(values = c("black", "turquoise"))+
  scale_colour_manual(values = c("black", "turquoise"))
# "CpGs covered in >=15 datasets"

table(is.na(Brent2$alpha))
## Save for comparison with Atlas

write.table(Brent2, "results_MariasarraysREDUCED_3samples_15datasets_6906CpGs_0_8p0_0_65p1.tsv", quote = F, row.names = F)
```
