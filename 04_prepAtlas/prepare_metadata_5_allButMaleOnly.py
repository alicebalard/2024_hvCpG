...
# Keep groups where NOT all rows are 'M'
df_filtered = df.groupby("Analysis group").filter(lambda g: not (g["sex"] == "M").all())

# Determine output path
meta_out = args.output if args.output else os.path.splitext(args.meta)[0] + "_5_allButMaleOnly.csv"
