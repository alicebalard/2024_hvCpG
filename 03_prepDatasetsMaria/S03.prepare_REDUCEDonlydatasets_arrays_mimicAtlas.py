...
- Merge them
- Create a reduced HDF5 matrix where each CpG has exactly `nDatasets` non-NaN values, for all available samples
...
def process_reduced_h5(nDatasets, output_root="/home/alice/arraysh5_reducedMimicAtlas"):
    # --- PARAMETERS ---
    ...
    output_folder = f"{output_root}_allsamples_{nDatasets}datasets"
    ...
    for i, cpg in enumerate(sorted_common_cpgs):
        ...
        if len(datasets) < nDatasets:
            continue  # Still skip CpGs not covered in enough datasets
...
## NB for memory reson, do it by block
for n in range(1, 31):  # last is exclusive
    print(f"🚀 Running for nDatasets = {n}")
    process_reduced_h5(nDatasets=n)
