#!/usr/bin/env python3
"""
- Load .RDS files from CD4+ and CD8+ arrays
- Merge them
...
output_folder = "/home/alice/arraysh5_CD4+CD8+"
...
rds_files = [
    os.path.join(folder1, "CD4+_Estonian.RDS"),
    os.path.join(folder1, "CD8+_Estonian.RDS")
]

rds_list_mat = load_rds_matrix(rds_files)

print(f"✅ Total datasets loaded: {len(rds_list_mat)}")

# --- STEP 2: Identify common CpGs (covered in ≥2 datasets) ---
all_cpgs = []
for _, _, rownames in rds_list_mat:
    all_cpgs.extend(rownames)

cpg_counts = pd.Series(all_cpgs).value_counts()
common_cpgs = set(cpg_counts[cpg_counts >= 2].index.tolist())
sorted_common_cpgs = sorted(common_cpgs)
print(f"✅ {len(sorted_common_cpgs):,} CpGs covered in ≥2 datasets")
...
#📖 Reading: /mnt/old_user_accounts/p3/maria/PhD/Data/datasets/GEO/BMIQ + 10 PCs + age + sex OUTLIERS REMOVED/CD4+_Estonian.RDS
#📖 Reading: /mnt/old_user_accounts/p3/maria/PhD/Data/datasets/GEO/BMIQ + 10 PCs + age + sex OUTLIERS REMOVED/CD8+_Estonian.RDS
#✅ Total datasets loaded: 2
#✅ 352,914 CpGs covered in ≥2 datasets
#/home/alice/2024_hvCpG/03_prepDatasetsMaria/S03.prepare_h5files_MariaArrays_CD4+CD8+.py:90: RuntimeWarning: invalid value encountered in cast
#  mat_filtered = df_filtered.values.astype(np.float32)
#✅ CD4+_Estonian: median_sd = 0.0162, lambda = 2.6715
#✅ CD8+_Estonian: median_sd = 0.0183, lambda = 2.6631
#✅ Saved matrix to: /home/alice/arraysh5_CD4+CD8+/all_matrix_noscale.h5
#✅ Saved metadata to: /home/alice/arraysh5_CD4+CD8+/sample_metadata.tsv
#✅ Saved stats to: /home/alice/arraysh5_CD4+CD8+/all_medsd_lambda.tsv
