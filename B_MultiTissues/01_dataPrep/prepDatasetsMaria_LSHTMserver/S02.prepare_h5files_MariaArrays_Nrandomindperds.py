...
Usage:
python3 S02.prepare_h5files_MariaArrays_Nrandomindperds.py --n_ind 5
...
# --- ARGS ---
parser = argparse.ArgumentParser(description="Build all_matrix_noscale.h5 with N random individuals per dataset.")
parser.add_argument("--n_ind", type=int, default=3,
                     help="Number of individuals to randomly keep per dataset (default: 3).")
...
output_folder = f"/home/alice/arraysh5_{N_IND}ind"
...
# Keep only N_IND random individuals (columns) per dataset ---
np.random.seed(1234)  # fix seed for reproducibility
reduced_list_mat = []
for name, mat, rownames in rds_list_mat:
    if mat.shape[1] > N_IND:  # only reduce if more than N_IND individuals
        cols = np.random.choice(mat.shape[1], size=N_IND, replace=False)
        mat = mat[:, cols]
    reduced_list_mat.append((name, mat, rownames))

rds_list_mat = reduced_list_mat
print(f"✅ Reduced to {N_IND} individuals per dataset")
