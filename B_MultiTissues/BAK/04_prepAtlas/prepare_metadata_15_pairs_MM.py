#!/usr/bin/env python3
"""
Prepare metadata for WGBS Atlas analysis (Pairs: MM)
Select exactly 2 males per Analysis group (if possible).
...
def main():
    parser = argparse.ArgumentParser(description="Prepare metadata pairs: MM")
    ...
    # Eligible groups: groups with at least 2 males
    selected = []
    eligible_groups = []
    ...
    # From each eligible group, pick exactly 2 random males
    for grp in eligible_groups:
        g = df[df["Analysis group"] == grp]
        males = g[g["sex"] == "M"]
        pick_idx = rng.choice(males.index, size=2, replace=False)
    ...
    # Default output name
    meta_out = args.output if args.output else os.path.splitext(args.meta)[0] + "_5_pairs_MM.csv"
    out_df.to_csv(meta_out, index=False)

    print(f"✅ Pairs MM: {out_df['Analysis group'].nunique()} groups, {len(out_df)} samples")
