...
me_sets <- list(
  Derakhshan = DerakhshanhvCpGs_hg38_GR,
  Harris     = HarrisSIV_hg38_GR,
  VanBaakESS = VanBaakESS_hg38_GR,
  VanBaakSIV = VanBaakSIV_hg38_GR,
  Kessler    = KesslerSIV_GRanges_hg38,
...
# 
# # Pr(HV) per layer + category from wideFull (rename name -> cpg_site in the merge)
# scored <- merge(uni,
#                 wideFull[, .(cpg_site = name, endo, meso, ecto, category)],
#                 by = "cpg_site", all.x = TRUE)
# 
# scored[, alpha_min := pmin(endo, meso, ecto, na.rm = TRUE)]   # ME = high in ALL
# scored[, alpha_geo := exp(rowMeans(log(cbind(endo, meso, ecto)), na.rm = TRUE))]
# 
# scored[, control := fcase(
#   is_ME_hi,                   "ME_hi",
#   n_catalogs == 1,            "ME_weak",
#   category == "constitutive", "constitutive",
#   category == "ambiguous",    "ambiguous",
#   default =                   "other")]
# 
# table(scored$control)
# # ambiguous constitutive        ME_hi      ME_weak        other 
# # 10190015     10887504        14925       102463       327634 
...
plot_quadrant_layer <- function(wide, HV = HVt, notHV = notHVt, subsampling = 50000) {
  make_plot <- function(x_col, y_col, x_lab, y_lab, title) {
    ...
    ggplot(wide, aes(x = .data[[x_col]], y = .data[[y_col]],
      ...
      geom_point(data = wide[!category %in% c("constitutive","ambiguous")],
                 alpha = 0.4, size = 1) +
...
ggplot2::ggsave(
  filename = here("B_MultiTissues/dataOut/figures/script10/plot_quadrant_layer_wideFull.pdf"),
  plot     = plot_quadrant_layer(wideFull), width    = 7, height = 7)

ggplot2::ggsave(
  filename = here("B_MultiTissues/dataOut/figures/script10/plot_quadrant_layer_wide6gpall.pdf"),
...
extract_layer_specific_cpgs <- function(meth_sub,
                                        ...
                                        r_high = NULL,      # "high" bar for own layer
                                        r_low  = NULL,      # "low"  bar 
...
fwrite(ME_res$gene_cpgs, here("B_MultiTissues/dataOut/ME_hits_genesClusters.csv"))
