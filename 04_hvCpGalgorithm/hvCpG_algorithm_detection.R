...
getAllOptimAlpha <- function(cpgvec){
  res = lapply(cpgvec, runOptim1CpG)
  names(res)=cpgvec
...
system.time(res_allOptimAlphapar <- getAllOptimAlpha_parallel(cpgvec = testCpGs200, NCORES=NCORES))
saveRDS(res_allOptimAlphapar, file = paste0("res_allOptimAlphapar_",analysisName, ".RDS"))
